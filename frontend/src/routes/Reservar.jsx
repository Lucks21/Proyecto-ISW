...
import { Tabs, Popover, Spinner } from "flowbite-react";
import {
  HiOutlinePuzzle,
        ...
        <Tabs.Item active title="Implemento" icon={HiOutlinePuzzle}>
          <div className="flex justify-around">
            <div className="m-4 w-full max-w-2xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 overflow-y-scroll h-[42rem] custom-scrollbar">
              ...
              <div className="flow-root">
                <ul className="divide-y divide-gray-200">
                  {implementos.map((imp) => (
                    <li
                      className={`py-3 sm:px-4 rounded-md ${
                        imp._id === currentImp?._id ? "bg-emerald-100" : ""
                      }`}
                      key={imp._id}
                    >
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {imp.estado === "disponible" ? (
                              <button
                                className="bg-blue-200 rounded-full p-2 hover:bg-blue-400"
                                title="reservar"
                                onClick={() => setCurrentImpl(imp)}
                              >
                                <HiOutlineCalendar />
                              </button>
                            ) : (
                              <button
                                ...
                                title="solicitud"
                                onClick={() =>
                                  handleSolicitud(imp, "implemento")
                                }
                              >
                                Solicitud
                              </button>
            ...
            {currentImp && (
              <HoursCardImp
                imp={currentImp}
                onReserva={(reservaData) => handleReserva(reservaData, currentImp)}
              />
            )}
          </div>
        </Tabs.Item>
        <Tabs.Item title="Instalación" icon={HiOutlineOfficeBuilding}>
          <div className="flex justify-around">
              ...
              <div className="flow-root">
                <ul className="divide-y divide-gray-200">
                  {instalaciones.map((ins) => (
                    <li
                      className={`py-3 sm:px-4 rounded-md ${
                        ins._id === currentInst?._id ? "bg-emerald-100" : ""
                      }`}
                      key={ins._id}
                    >
                      <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            {ins.estado === "disponible" ? (
                              ...
                              <button
                                ...
                                title="solicitud"
                                onClick={() =>
                                  handleSolicitud(ins, "instalacion")
                                }
                              >
                                Solicitud
                              </button>
            ...
            {currentInst && (
              <HoursCardImp
                imp={currentInst}
                onReserva={(reservaData) => handleReserva(reservaData, currentInst)}
              />
            )}
          </div>
        </Tabs.Item>
